...
class Contenedor {
  ...
  async save(objeto) {
    ...
    try {
      if (!data) {
        let id = JSON.parse(await fs.promises.readFile("./Clase4/ids.txt", "utf-8"));
        let maxID = Math.max(...id);
        objeto.id = maxID + 1;
        id = [...id, objeto.id];
        await fs.promises.writeFile(`./Clase4/ids.txt`, JSON.stringify(id));
        ...
        let productos = JSON.parse(
          await fs.promises.readFile("./Clase4/productos.txt", "utf-8")
        );
...
//new Contenedor('./Clase4/productos.txt').save({nombre: 'shampoo', precio: 200, thumbnail: 'ejemplo url'})

//new Contenedor('./Clase4/productos.txt').getById(12)

//new Contenedor('./Clase4/productos.txt').getAll()

//new Contenedor('./Clase4/productos.txt').deleteById(12)

//new Contenedor('./Clase4/productos.txt').deleteAll()